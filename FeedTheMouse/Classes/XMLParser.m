...
- (XMLParser *) initXMLParser {
    ...
    bombs = [[NSMutableArray alloc] initWithCapacity:3];
...
- (void) parser: (NSXMLParser *)parser foundCharacters:(NSString *)string {
   ...
   // printf("string: %s", [string UTF8String]);
    if ([currentElementName isEqualToString:@"Background"])
        backgroundFilename = [[NSString alloc] initWithString:string];
...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"Background"])
    {
        level->backgroundFilename = [[NSString alloc] initWithString:backgroundFilename];
    }
    else if ([elementName isEqualToString:@"Level"])
    {
        ...
        {
            Bomb *newBomb = [[[Bomb alloc] init] autorelease];
            Bomb *b = (Bomb*)[bombs objectAtIndex:i];
            int bx = [b getX];
            int by = [b getY];
            float newAngle = [b getAngle];
            [newBomb initializeBombAtX:bx andY:by andAngle:newAngle];
            [level addBomb:newBomb];
        }
